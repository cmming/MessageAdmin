                ...
                <table class="table table-responsive table-condensed table-bordered table-striped table-hover">
                    ...
                    <tbody>
                        <tr v-for="(item,key) in dataList">
                            ...
                            <td v-show="tableHeader[1].val">{{item.recvid}}</td>
                            <td v-show="tableHeader[2].val">{{item.title}}</td>
                            <td v-show="tableHeader[3].val">{{item.is_send |is_send(item.is_send)}}</td>
                        </tr>
                    </tbody>
                </table>
                <!--分页组件-->
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                var self = this;
                var resData = { 'type': "getMsg", 'dataform': JSON.stringify({'cur_page':this.curpage})};
